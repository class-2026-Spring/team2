  ...
  return (
    <div style={{ padding:"16px 16px 8px" }} className="fi">
      <div style={{ background:`linear-gradient(135deg,${C.pD} 0%,${C.p} 55%,#06B6D4 100%)`, borderRadius:22, padding:"22px 20px 20px", marginBottom:16, position:"relative", overflow:"hidden" }}>
        ...
        <button onClick={() => window.open("tel:119")} style={{ background:"#fff", color:C.red, border:"none", borderRadius:13, padding:"12px 20px", fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:fs*16, cursor:"pointer", boxShadow:"0 4px 16px rgba(220,38,38,.32)", display:"inline-flex", alignItems:"center", gap:6 }}>
          {s.call}
        </button>
      </div>
      ...
      <div style={{ background:C.pXL, borderRadius:13, padding:"11px 14px", border:`1px solid ${C.border}`, fontSize:fs*12, color:C.text, lineHeight:1.55 }}>
        {s.tip}
      </div>
      <div style={{ textAlign:"center", fontSize:fs*10.5, color:C.sub, marginTop:14, padding:"0 4px", lineHeight:1.5 }}>
